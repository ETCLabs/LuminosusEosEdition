...
AudioInputAnalyzer::AudioInputAnalyzer(QAudioDeviceInfo inputInfo, MainController* controller)
    : QObject(controller)
    , m_controller(controller)
    , m_referenceList()
    , m_audioInput(nullptr)
    , m_audioRecordDevice(nullptr)
    , m_deviceName(inputInfo.deviceName())
    , m_circBuffer(CIRC_BUFFER_LENGTH)
    , m_analyzed_till_index(0)
...
AudioInputAnalyzer::AudioInputAnalyzer(MainController* controller)
    : QObject(controller)
    , m_controller(controller)
    , m_referenceList()
    , m_audioInput(nullptr)
    , m_audioRecordDevice(nullptr)
    , m_deviceName("No Input")
    , m_circBuffer(0)
    , m_analyzed_till_index(0)
    , m_spectrumHistory(SPECTRUM_HISTORY_LENGTH)
    , m_newSpectrumCount(0)
...
void AudioInputAnalyzer::addReference(void* ref) {
    if (m_referenceList.isEmpty() && m_audioInput) {
...
void AudioInputAnalyzer::audioDataReady() {
    ...
    const int bytesPerSample = m_audioFormat.sampleSize() * m_audioFormat.channelCount() / 8;
    ...
    m_analyzed_till_index = qMax(0, m_analyzed_till_index - numSamples);
...
void AudioInputAnalyzer::analyzeNewSamples() {
    m_newSpectrumCount = 0;
    while (CIRC_BUFFER_LENGTH - m_analyzed_till_index >= SAMPLES_BETWEEN_SPECTRUM_UPDATES) {
        createSpectrumTillIndex(m_analyzed_till_index + SAMPLES_BETWEEN_SPECTRUM_UPDATES);

        m_analyzed_till_index += SAMPLES_BETWEEN_SPECTRUM_UPDATES;
    }
    updateOnsets();
...
void AudioInputAnalyzer::createSpectrumTillIndex(int endIndex) {
    if (endIndex < LONG_NUM_SAMPLES) return;
    updateRawSpectrumsTillIndex(endIndex);
    createSimplifiedSpectrumFromRawSpectrums();
}

void AudioInputAnalyzer::updateRawSpectrumsTillIndex(int endIndex) {
...
void AudioInputAnalyzer::createSimplifiedSpectrumFromRawSpectrums() {
    ...
    for (int i = 0; i<SIMPLIFIED_SPECTRUM_LENGTH; ++i) {
        ...
        if (freq < 200) {
            const int startIndex = freq / 22050 * (LONG_NUM_SAMPLES / 2);
            const int endIndex = nextFreq / 22050 * (LONG_NUM_SAMPLES / 2);
            const int valuesTillNext = endIndex - startIndex;
            if (endIndex >= LONG_NUM_SAMPLES) continue;
            ...
            const int startIndex = freq / 22050 * (SHORT_NUM_SAMPLES / 2);
            const int endIndex = nextFreq / 22050 * (SHORT_NUM_SAMPLES / 2);
            const int valuesTillNext = endIndex - startIndex;
    ...
    m_spectrumHistory.push_back(simplifiedSpectrum);
    ++m_newSpectrumCount;
    ...
    double maxComponent = qMax(redMax, qMax(greenMax, blueMax));
    if (maxComponent == 0) {
...
int CLUSTER_WIDTH_IN_FRAMES = msToFrames(CLUSTER_WIDTH); // frames
        ...
        if (m_onsetBuffer[i]) {
            for (int j = i+1; j < SPECTRAL_FLUX_HISTORY_LENGTH; ++j) {
                if (m_onsetBuffer[j]) {
                    ...
                    // Detect the interval and score, and continue on if the interval is to short or to long
...
BeatAgent* AudioInputAnalyzer::plausibleAgentForInterval(float interval, float maxScore) {
    for (BeatAgent& agent : m_agents) {
        if (qAbs(agent.getAverageInterval() - interval) < CLUSTER_WIDTH) {
            if (agent.getScore() >= 0.75 * maxScore) {
                return &agent;
            }
        }
    }
...
static const float fractionsToCheck[] = {4.0/3.0,
                                         2.0/3.0,
                                         2.0,
                                         0.5,
                                         3.0};
