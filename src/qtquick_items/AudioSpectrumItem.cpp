...
QPointF AudioSpectrumItem::normalFromTangent(const QPointF& tangent) {
...
QSGNode* AudioSpectrumItem::updatePaintNode(QSGNode* oldNode, UpdatePaintNodeData*) {
    ...
    for (int i = 0; i < pointCount; ++i) {
        ...
        if (i > 0 && i < pointCount - 1) {
            ...
            // m is slope (Steigung)
            const double prevM = (points[i] - points[i-1]) * itemHeight;
            const double nextM = (points[i+1] - points[i]) * itemHeight;
