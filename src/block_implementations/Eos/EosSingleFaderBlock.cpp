...
void EosSingleFaderBlock::getAdditionalState(QJsonObject& state) const {
    state["page"] = getPage();
    state["faderNumber"] = getFaderNumber();
    state["value"] = m_faderLevels[m_faderNumber - 1];
}

void EosSingleFaderBlock::setAdditionalState(const QJsonObject& state) {
    setPageFromGui(state["page"].toInt());
    setFaderNumber(state["faderNumber"].toInt());
    setFaderLevelFromGui(state["value"].toDouble());
}
