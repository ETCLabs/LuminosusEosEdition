	...
	StretchColumn {
		...
		BlockRow {
			...
			ButtonBottomLine {
                ...
                onTouchDown: controller.customOsc().enableLogging(logIncomingCheckbox.active, logOutgoingCheckbox.active)
				text: "Incoming"
				toggle: true
				//textColor: "#88d"
			}
			ButtonBottomLine {
				...
				onTouchDown: {
                    controller.customOsc().enableLogging(logIncomingCheckbox.active, logOutgoingCheckbox.active)
				}
				text: "Outgoing"
				toggle: true
			}
			ButtonBottomLine {
				text: "Clear"
                onPress: controller.customOsc().clearLog()
			}
            HeightResizeArea {
                width: 30*dp
                target: root
			...
			Component.onCompleted: {
                ...
                logIncomingCheckbox.active = controller.customOsc().getLogIncomingIsEnabled()
                logOutgoingCheckbox.active = controller.customOsc().getLogOutgoingIsEnabled()
			}
		}
        ...
        CustomTouchArea {
			...
			ListView {
				id: logList
                model: controller.customOsc().getLog()
				clip: true
				...
				Connections {
                    target: controller.customOsc()
					onLogChanged: {
                        logList.model = controller.customOsc().getLog()
					}
				}
			}  // end ListView
