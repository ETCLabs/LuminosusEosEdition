...
void NodeBase::setGuiItem(QQuickItem* item) {
    ...
    m_guiItem = item;
    if (isOutput()) {
        m_connectionLines = item->findChild<NodeConnectionLines*>("connectionLines");
        if (m_connectionLines) {
            m_connectionLines->setNodeObject(this);
        }
    }
    connect(item, SIGNAL(touched()), this, SLOT(onTouched()));
    if (!m_isOutput) {
        connect(item, SIGNAL(longClick()), this, SLOT(toggleHtpMode()));
    }
}
...
void NodeBase::focus() {
    s_focusedNode = this;
    if (m_guiItem) m_guiItem->setProperty("focused", true);
}

void NodeBase::defocus() {
    if (s_focusedNode == this) {
        s_focusedNode = nullptr;
        if (m_guiItem) m_guiItem->setProperty("focused", false);
    }
}
...
void NodeBase::updateConnectionLines() {
    ...
    if (!m_connectionLines) return;
    m_connectionLines->update();
}
...
void NodeBase::setRequestedSize(QSize value) {
    if (m_isOutput) {
        qCritical() << "Method setRequestedSize() is only available for input nodes.";
        return;
    }
    if (value == m_requestedSize) return;  // value isn't changing
    if (value.isEmpty()) {
...
void NodeBase::setHtpMode(bool value) {
    ...
    if (m_guiItem) m_guiItem->setProperty("htp", value);
}
...
void NodeBase::updateData(NodeBase* ltpSource) {
    ...
    if (m_htp || !ltpSource) {
        ...
        for (NodeBase* outputNode: m_connectedNodes) {
            if (!outputNode) continue;
            const ColorMatrix& data = outputNode->getDataToRead();
            if (int(data.getSX()) < m_requestedSize.width()
                    || int(data.getSY()) < m_requestedSize.height()) {
        ...
        const ColorMatrix& data = ltpSource->getDataToRead();
        if (int(data.getSX()) < m_requestedSize.width()
                || int(data.getSY()) < m_requestedSize.height()) {
...
void NodeBase::updateRequestedSize() {
    ...
    for (NodeBase* inputNode: m_connectedNodes) {
        if (!inputNode) continue;
        m_requestedSize = m_requestedSize.expandedTo(inputNode->getRequestedSize());
    }
